pub mod baseline {
    ...
    const BASE_URL: &str = "https://api.ipbase.com/v2/";

    pub fn construct_client(
        user_agent: Option<&str>,
        settings: &api::Settings,
    ) -> Result<Client, IpbaseError> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|err| IpbaseError::ClientConstruction { source: err })?;
        Ok(client)
    }
    ...
    ) -> Result<Url, IpbaseError> {
        let mut url = Url::parse(BASE_URL).map_err(|_| IpbaseError::UrlConstruction)?;
